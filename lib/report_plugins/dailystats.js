...
dailystats.html = function html (client) {
  ...
  if (!hideMenu) {
  var ret = '<h2>' + translate('Daily stats report') + '</h2>';
  } else {
    var ret = '';  
  }

    ret += '<div id="dailystats-report"></div>';
...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  $('<th>' + translate('Readings') + '</th>').appendTo(thead); // moved
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    // Ensure Moment.js uses the Swedish locale for this instance
    var dayMoment = moment(day).locale('sv');
    var dayName = dayMoment.format('dddd'); // Will return day name in Swedish
    var dateStr = dayMoment.format('YYYY-MM-DD').replace(/-/g, '\u2011');

    dateStr = '<span style="white-space: nowrap;">' + dateStr + '</span>';
    var dateCellContent = dayName + '<br>' + dateStr;

$('<td class="tdborder" style="width:160px">' + dateCellContent + '</td>').appendTo(tr);
    $('<td class="tdborder">' + average.toFixed(1) + '</td>').appendTo(tr); // moved
    $('<td class="tdborder">' + ((100 * stats.lows) / daysRecords.length).toFixed(1) + '%</td>').appendTo(tr); //Daniel: Add 1 decimal for low %
    ...
    $('<td class="tdborder">' + ss.standard_deviation(bgValues).toFixed(1) + '</td>').appendTo(tr); // moved
    ...
    $('<td class="tdborder">' + daysRecords.length + '</td>').appendTo(tr); // moved
