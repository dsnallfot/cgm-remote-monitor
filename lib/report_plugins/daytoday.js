      ...
      sel.attr('cx', function(d) {
        ...
        .on('mouseover', function(d) {
          if (options.openAps && d.openaps) {
              ...
              var convertedbg = d.openaps.suggested.bg;
              if (units === 'mmol') {
                convertedbg = Math.round(bg / consts.MMOL_TO_MGDL * 10) / 10;
              }
      
              // Prepare the tooltip text
              var text = '<b>BG:</b> ' + convertedbg +
              ...
              client.tooltip.style('display', 'block');
             var text = '<b>BG:</b> ' + Math.round(d.openaps.suggested.bg / consts.MMOL_TO_MGDL * 10) / 10 + d.openaps.suggested.reason +
               (d.openaps.suggested.mealAssist ? ' <b>Meal Assist:</b> ' + d.openaps.suggested.mealAssist : '');
             client.tooltip.html(text)
               .style('left', (d3.event.pageX) + 'px')
               .style('top', (d3.event.pageY + 15) + 'px');
