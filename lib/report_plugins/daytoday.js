      ...
      sel.attr('cx', function(d) {
        ...
        .on('mouseover', function(d) {
          if (options.openAps && d.openaps) {
              ...
              // Extract and format the deliverAt timestamp in Swedish local time
            var deliverAtTime = '';
            if (d.openaps.suggested.deliverAt) {
                ...
                deliverAtTime = localTime.toLocaleTimeString('sv-SE', options);
            }

              var text = '<b>BG:</b> ' + Math.round(d.openaps.suggested.bg * 0.0555 * 10) / 10 + ' • ' + deliverAtTime + '<br> • ' + reasonWithLineBreaks + (d.openaps.suggested.mealAssist ? ' <b>Meal Assist:</b> ' + d.openaps.suggested.mealAssist : '');
