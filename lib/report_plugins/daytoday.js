        ...
        .on('mouseover', function(d) {
          if (options.openAps && d.openaps) {
            ...
            if (d.openaps.suggested.deliverAt) {
                ...
                deliverAtTime = localTime.toLocaleString('sv-SE', options).split(' ')[1];
            }
            
            var text = '<b>BG:</b> ' + Math.round(d.openaps.suggested.bg * 0.0555 * 10) / 10 + ' • ' + deliverAtTime + '<br>• ' + reasonWithLineBreaks +
                (d.openaps.suggested.mealAssist ? ' <b>Meal Assist:</b> ' + d.openaps.suggested.mealAssist : '');
