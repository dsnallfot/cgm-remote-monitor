      ...
      sel.attr('cx', function(d) {
        ...
        .on('mouseover', function(d) {  
          if (options.openAps && d.openaps) {
              ...
              // Store the original blood glucose value for positioning
              var originalBg = d.openaps.suggested.bg;
      
              // Get the user unit setting
              var units = sbx.data.profile.getUnits();
      
              // Convert the blood glucose value if the units are mmol/L
              var displayBg = originalBg;
              if (units === 'mmol') {
                  displayBg = Math.round(originalBg / consts.MMOL_TO_MGDL * 10) / 10;
              }
      
              // Prepare the tooltip text
              var text = '<b>Blodsocker:</b> ' + displayBg +
                  ', ' + d.openaps.suggested.reason +
                  (d.openaps.suggested.mealAssist ? ' <b>Meal Assist:</b> ' + d.openaps.suggested.mealAssist : '');
      
              // Use d3.mouse to get the mouse coordinates
              var coordinates = d3.mouse(this);

              // Set the tooltip content and position
              client.tooltip.html(text)
              .style('left', (coordinates[0] + padding.left) + 'px')
              .style('top', (coordinates[1] + padding.top + 15) + 'px');  
            }
        })
