...
daytoday.html = function html(client) {
  ...
  // Parse URL parameters inside the function to maintain access to 'client'
  var params = getQueryParams();
  var hideMenu = params['hideMenu'] === 'true';
  var hideStats = params['hideStats'] === 'true';
  ...
  // Date range setup
  var from, to;
  var fromDateStr = params['startDate'];
  var toDateStr = params['endDate'];

  if (hideMenu) {
    if (fromDateStr && toDateStr) {
      from = moment.tz(fromDateStr, zone).startOf('day');
      to = moment.tz(toDateStr, zone).endOf('day');

      if (fromDateStr === toDateStr) {
        to = moment.tz(toDateStr, zone).endOf('day');
      }
    } else {
      from = moment.tz(moment().startOf('day'), zone).startOf('day');
      to = moment.tz(moment().endOf('day'), zone).endOf('day');
    }
  }

  if (hideMenu) {
      return '<div id="daytodaycharts"></div>';
    } else {
  var ret =
    ...
    `<label><input type="checkbox" id="rp_optionsopenaps" ${reportStorage.getValue('openAps') ? "checked" : ""}><span style="color:sienna;opacity:0.5">${translate('Trio Algo')}</span></label>` +
    ...
    `<label><input type="checkbox" id="rp_optionspredicted" ${reportStorage.getValue('predicted') ? "checked" : ""}><span style="color:sienna;opacity:0.5">${translate('Prognoslinjer')}</span></label>` +
    ...
    `<label><input type="checkbox" id="rp_optionsothertreatments" ${reportStorage.getValue('othertreatments') ? "checked" : ""}>${translate('Övriga behandlingar')}</span></label>` +
    '&nbsp;' + translate('Size') +
    ' <select id="rp_size">' +
...
daytoday.report = function report_daytoday(datastorage, sorteddaystoshow, options) {
  ...
  // **Parse URL parameters before invoking drawChart**
  function getQueryParams() {
    var params = {};
    window.location.search.substring(1).split('&').forEach(function(pair) {
      var keyValue = pair.split('=');
  ...
  // Conditionally append insulin distribution HTML
  if (options.insulindistribution && !hideMenu) {
  ...
  // --- Engångs– ”plåster”  för basal­överskattningen -----------------
var INSULIN_OVERESTIMATION_PER_DAY = 1.5;                         // U/dygn
var insulinCorrection = INSULIN_OVERESTIMATION_PER_DAY * datastorage.alldays;

// Korrigera summorna men se till att de aldrig blir negativa
var correctedTddSum   = Math.max(0, tddSum   - insulinCorrection);
var correctedBasalSum = Math.max(0, basalSum - insulinCorrection);
// -------------------------------------------------------------------

// ----- Alla medelvärden & procentsatser beräknas nu på de justerade värdena -----
var tddAverage              = correctedTddSum   / datastorage.alldays;
var basalAveragePercent     = Math.round((correctedBasalSum / datastorage.alldays) / tddAverage * 100);
var baseBasalAveragePercent = Math.round((baseBasalSum      / datastorage.alldays) / tddAverage * 100); // lämnas oförändrad
var bolusAveragePercent     = Math.round((bolusSum          / datastorage.alldays) / tddAverage * 100);
var carbsAverage            = carbsSum   / datastorage.alldays;
var proteinAverage          = proteinSum / datastorage.alldays;
var fatAverage              = fatSum     / datastorage.alldays;

// ----- HTML-renderingen oförändrad (visar nu de korrigerade värdena) -------------
if (options.insulindistribution && !hideMenu) {
  let html  = '<br><br><b>' + translate('TDD average')   + ':</b> ' + tddAverage.toFixed(1) + 'U&nbsp; ';
  html     += '<b>' + translate('Bolus average')         + ':</b> ' + bolusAveragePercent + '%&nbsp; ';
  html     += '<b>' + translate('Basal average')         + ':</b> ' + basalAveragePercent + '%&nbsp; ';
  html     += '<b>(' + translate('Base basal average:')  + '</b> ' + baseBasalAveragePercent + '%<b>)</b>&nbsp; <br>';
  html     += '<b>' + translate('Carbs average')         + ':</b> ' + carbsAverage.toFixed(0)   + 'g  ';
  html     += '<b>' + translate('Protein average')       + ':</b> ' + proteinAverage.toFixed(0) + 'g  ';
  html     += '<b>' + translate('Fat average')           + ':</b> ' + fatAverage.toFixed(0)     + 'g  ';
  $('#daytodaycharts').append(html);
  ...
  function drawChart (day, data, options) {
    ...
    function prepareContextCircles (sel) {
      ...
      sel.attr('cx', function(d) {
        ...
        // Instead of overall opacity, set fill opacity only so the stroke stays fully opaque
        ...
        .attr('stroke', 'black')
        // (Optional) If you want to be explicit, you can set stroke opacity
        .attr('stroke-opacity', function(d) {
            return (options.openAps && d.openaps) ? 1 : 0.5;
        })
        .attr('r', function(d) {
          if (d.type === 'mbg') {
            return 4;
    ...
    */
        /*.on('mouseover', function(d) {
          if (options.openAps && d.openaps) {
             ...
             var text = '<b>BG:</b> ' + Math.round(d.openaps.suggested.bg * 0.0555 * 10) / 10 + ' • ' + d.openaps.suggested.reason +
        ...
        var reasonWithLineBreaks = d.openaps.suggested.reason;

              // ✅ Step 1: Handle AF and SMB Ratio *before* replacing anything else
        reasonWithLineBreaks = reasonWithLineBreaks.replace(/AF:\s([0-9]\.\d{1,2})(?:,\sSMB Ratio:\s([0-9]\.\d{1,2}))?;/g, function(match, afValue, smbRatio) {
          let result = `AF: ${afValue} <br>`; // AF always appears first
          if (smbRatio) {
              result += `• SMB Ratio: ${smbRatio} <br>`; // If SMB Ratio exists, place it directly after AF
          }
          return result + `------------------------------ <br><b>☑️ Oref slutsats:</b><br> • `; // Ensure this is added once at the correct position
      });

      // ✅ Step 2: Replace all commas with line breaks *after* AF/SMB has been handled
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/,/g, ' <br> • ');

      // ✅ Step 3: Continue with other replacements (won't affect AF/SMB placement)
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/SMB INAKTIVERADE!/g, 'SMB Inaktiverade 🚫');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/Mikrobolus:/g, '🔹 Mikrobolus:');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/\. ;/g, '<br>• ');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/E\. /g, 'E<br>');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/U\. /g, 'E<br>');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/E\/h\. /g, 'E/h<br>');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/temp\./g, 'temp.<br>');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/\. /g, '');

      // ✅ Step 4: Replace "; " with <br> • (AF & SMB Ratio are already handled, so they won't be affected)
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/;\s+(?!\w*;|&\w+;)/g, ' <br> • ');

      // ✅ Step 5: Other formatting rules
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/add'l carbs req w\/in/g, 'g kh behövs inom');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/TDD:\s(\d+(?:\.\d{1,2})?)\sU/g, '<b>TDD: $1E</b>');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/temp\s([0-9]+\.\d{1,2})&lt;([0-9]+\.\d{1,2})U\/hr\./g, 'Temp $1&lt;$2E/h');
      reasonWithLineBreaks = reasonWithLineBreaks.replace(/insulinReq/g, 'Insulinbehov:');

              // Extract and format the deliverAt timestamp in hh:mm:ss
              var deliverAtTime = '';
              if (d.openaps.suggested.deliverAt) {
                var localTime = new Date(d.openaps.suggested.deliverAt);
                deliverAtTime = localTime.getHours().toString().padStart(2, '0') + ":" +
                                localTime.getMinutes().toString().padStart(2, '0') + ":" +
                                localTime.getSeconds().toString().padStart(2, '0');
              }     
              /*var text = '<b>BG:</b> ' + Math.round(d.openaps.suggested.bg * 0.0555 * 10) / 10 + 
                       ' • ' + deliverAtTime + '<br> • ' + reasonWithLineBreaks;*/

              var text = '<b>Trio status kl: </b> <b>' + deliverAtTime + '</b><br> • <b>BG:</b> <b>' + Math.round(d.openaps.suggested.bg * 0.0555 * 10) / 10 + '</b><br> • ' + reasonWithLineBreaks;
    ...
    data.treatments.forEach(function(treatment) {
      ...
      if (treatment.carbs && options.carbs) {
        ...
        // Daniel: hide foodType to declutter report // label += treatment.foodType ? ' ' + treatment.foodType : ''
        label += treatment.absorptionTime ? ' ' + (Math.round(treatment.absorptionTime / 60.0 * 10) / 10) + 'h' : ''
        // Daniel: Comment out below  to hide fat to declutter report 
        if (treatment.fat) label += ' / ' + treatment.fat + ' g';
        // Daniel: Comment out below to hide protein to declutter report 
      ...
      if (treatment.insulin && options.insulin) {
        var dataLabel = client.utils.toRoundedStr(treatment.insulin, 2)+ 'E';
        context.append('rect')
      ...
      if (treatment.insulin || treatment.carbs || treatment.eventType == 'Temp Basal' || treatment.eventType == 'Combo Bolus') {
        ...
        context.append('rect')
          ...
          .attr('stroke', 'white')
          .attr('fill', 'Violet');
        //Daniel: Regexing away any text in the Exercise Overrides, just keep the prefix emoji
          context.append('text')
          ...
          .text(() => {
            const match = treatment.notes.match(/^\p{Emoji_Presentation}+/u); // Match only the emoji at the start
            return match ? match[0] : ''; // Return the emoji or an empty string
          });
/*
          context.append('text')
          .style('font-size', '12px')
  ...
  */
    ...
    if (options.insulindistribution) {
      ...
      if (options.insulindistribution && !hideStats) {
