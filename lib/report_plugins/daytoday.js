      ...
      sel.attr('cx', function(d) {
        ...
        .on('mouseover', function(d) {
          if (options.openAps && d.openaps) {
              ...
              // Ensure "------------------------------ <br> ☑️ Oref slutsats:" appears after AF or SMB Ratio
              reasonWithLineBreaks = reasonWithLineBreaks.replace(/AF:\s([0-9]\.\d{1,2})(,\sSMB Ratio:\s([0-9]\.\d{1,2}))?/g, function(match, afValue, _, smbRatio) {
              let result = `AF: ${afValue} <br>`; // Always show AF first
              if (smbRatio) {
              result += `• SMB Ratio: ${smbRatio} <br>`; // SMB Ratio should appear directly after AF
              }
              result += `------------------------------ <br><b>☑️ Oref slutsats:</b>`;
              return result;
              });
