      ...
      sel.attr('cx', function(d) {
        ...
        .on('mouseover', function(d) {
          if (options.openAps && d.openaps) {
              ...
              // Apply regex replacements to d.openaps.suggested.reason
              ...
              // Make "Microbolusing" bold and add a line break
              ...
              // Replace "U." with U+line break
              reasonWithLineBreaks = reasonWithLineBreaks.replace(/U\. /g, 'U<br>  ');

              // Ensure "------------------------------ <br> ☑️ Oref slutsats:" appears at the right place
reasonWithLineBreaks = reasonWithLineBreaks.replace(/AF:\s([0-9]\.\d{1,2})(<br> • SMB Ratio:\s([0-9]\.\d{1,2}))?/g, function(match, afValue, smbRatio) {
  ...
  return result + `------------------------------ <br><b>☑️ Oref slutsats:</b>`; // Ensure this is added only once at the correct position
});

              // Now replace "; " with <br> • (AF & SMB Ratio are already handled)
              reasonWithLineBreaks = reasonWithLineBreaks.replace(/;\s+(?!\w*;|&\w+;)/g, ' <br> • ');

              // Replace "add'l carbs req w/in" with "g kh behövs inom"
              reasonWithLineBreaks = reasonWithLineBreaks.replace(/add'l carbs req w\/in/g, 'g kh behövs inom');

              // Replace "TDD: x.x U" or "TDD: x.xx U" with bold formatting and "E" instead of "U"
              reasonWithLineBreaks = reasonWithLineBreaks.replace(/TDD:\s(\d+\.\d{1,2})\sU/g, '<b>TDD: $1 E</b>');
