...
var DEVICE_TYPE_FIELDS = ['uploader', 'pump', 'openaps', 'Trio', 'loop', 'xdripjs'];
...
function init () {
  ...
  ddata.processTreatments = function processTreatments (preserveOrignalTreatments) {
    ...
    // Daniel: filter & prepare 'Exercise' events
    var exerciseTreatments = ddata.treatments.filter(function filterExercise (t) {
      return t.eventType === 'Exercise';
    });
    if (preserveOrignalTreatments)
      exerciseTreatments = _.cloneDeep(exerciseTreatments);

    // Process overlaps for 'Exercise' events
    ddata.exerciseTreatments = ddata.processDurations(exerciseTreatments, false);
