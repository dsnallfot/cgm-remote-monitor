...
var DEVICE_TYPE_FIELDS = ['uploader', 'pump', 'openaps', 'iaps', 'loop', 'xdripjs'];
    ...
    var rv = _.chain(deviceAndTypes)
      .map(function findMostRecent (deviceAndType) {
        return _.chain(ddata.devicestatus)
          ...
          //Daniel: Test to included up to 1 minute delayed entries
          .filter(function includeDelayed(status) {
            // Adjust time by adding one minute
            var adjustedTime = time + (1 * 60 * 1000); // Convert 1 minute to milliseconds
