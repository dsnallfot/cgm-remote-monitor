...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    function fillColor (d) {
      ...
      if (d.eventType === 'Exercise') {
        ...
        color = 'Bright Cyan'; //Daniel sync color with iAPS
      }
      return color;
    }
  ...
  function prepareArc (treatment, radius, bolusSettings) {
    var arc_data = [
      // white carb half-circle on top // Daniel: Changed to yellow to match iAPS & Loop Follow
      { 'element': '', 'color': 'Amber', 'start': -1.5708, 'end': 1.5708, 'inner': 0, 'outer': radius.R1 }
