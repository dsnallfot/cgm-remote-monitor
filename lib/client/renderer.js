...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    function focusCircleTooltip (d) {
      ...
      client.tooltip.html('<strong>' + translate('BG') + ':</strong> ' + client.sbx.scaleEntry(d) +
          ...
          (d.type === 'forecast' && d.forecastType ? '<br/><strong>' + translate('Prognos') + ': </strong>' + d.forecastType : '') +
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    function treatmentTooltip (d) {
      ...
      if (d.durationType === "indefinite") {
        ...
        if (durationMinutes > 0 && durationMinutes % 60 == 0) {
          ...
          if (durationHours > 1) {
            durationText = durationHours + ' timmar';
          } else {
            durationText = durationHours + ' timme';
          }
        } else {
      ...
      return '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '<br/>' +
        ...
        (d.targetBottom ? '<strong>' + translate('Målvärde') + ':</strong> ' + targetBottom + '<br/>' : '') +
    ...
    //Daniel: Added same processing for exercise events
    durationTreatments = durationTreatments.concat(client.ddata.tempTargetTreatments, client.ddata.exerciseTreatments);
    ...
    function fillColor (d) {
      ...
      if (d.eventType === 'Exercise') {
        ...
        color = '#0affff'; //Daniel sync color with Trio
      }
      return color;
    }
...
*/
    // Daniel: Just show emoji for Exercise Trio Overrides
    function treatmentText(d) {
      if (d.eventType === 'Temporary Target') {
        return '';
      }
      if (d.eventType === 'Exercise' && d.notes) {
        const match = d.notes.match(/^\p{Emoji_Presentation}/u); // Match only the first emoji
        return match ? match[0] : ''; // Return the emoji if found, otherwise an empty string
      }
      return d.notes || d.reason || d.eventType;
    }
  ...
  function prepareArc (treatment, radius, bolusSettings) {
    var arc_data = [
      // white carb half-circle on top // Daniel: Changed to yellow to match Trio & Loop Follow, didnt look good, back to white
    ...
    // Daniel: Comment out fat & protein to declutter chart. Still visible as note on the carb dot
/*
    if (treatment.fat > 0) {
...
*/
    ...
    // Daniel: Comment out foodType to declutter chart. Still visible as note on the carb dot
    ...
    if (treatment.insulin > 0) {
      ...
      var unit_of_measurement = (format === 'minimal' ? '' : ' E'); // One international unit of insulin (1 IU) is shown as '1 E'
...
*/ // Daniel: Testing offset upwards
    var treatmentDots = chart().focus.selectAll('treatment-insulincarbs')
      ...
      .attr('transform', function(d) {
        var x = chart().xScale(getOrAddDate(treatment));
        var y = chart().yScale(client.sbx.scaleEntry(treatment)) - 50; // Offset by 50 pixels
...
*/
        .attr('transform', function(d) {
          d.outerRadius = d.outerRadius * 2.1;
          d.innerRadius = d.outerRadius * 2.1;
          var centroid = arc.svg.centroid(d);
          centroid[1] -= 50; // Offset by 50 pixels upwards
    ...
    _.forEach(client.ddata.tempbasalTreatments, function eachTemp (t) {
      ...
      if (t.duration && t.mills < to && t.mills + times.mins(t.duration).msecs > from) {
        var text = g.append('text')
          ...
          .attr('y', 10)
          .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (isNaN(t.absolute) ? '' : Number(t.absolute).toFixed(2) + 'E') + (t.relative ? 'C: +' + t.relative + 'E' : ''));
