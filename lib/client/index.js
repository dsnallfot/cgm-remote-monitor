...
_.each(client.sbx.pluginBase.forecastInfos, function eachInfo(info) {
  ...
  // Invert the logic for setting the "checked" property
  ...
  forecastLabel.append('<span>Hide ' + info.label + '</span>');
  ...
  forecastCheckbox.change(function onChange(event) {
    ...
    // Invert the logic for adding/removing items from client.settings.showForecast
    if (checked) {
...
// Daniel: Testing 9 min purge instead of default 5 -> revert to 5 again
  ...
  // Testing 1 min reload instead of default 30 sec
  client.loadRetroIfNeeded = function loadRetroIfNeeded () {
    var now = Date.now();
    if (now - client.retro.loadStartedMills < times.mins(1).msecs) {
      console.info('retro already loading, started', new Date(client.retro.loadStartedMills));
      return;
    }
    
// Daniel: Testing 2 min reload instead of default 3
    if (now - client.retro.loadedMills > times.mins(2).msecs) {
