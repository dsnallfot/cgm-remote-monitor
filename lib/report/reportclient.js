...
var init = function init () {
  ...
  client.init(function loaded () {
    ...
    function prepareGUI () {
      ...
      if (hideMenu) {
        // Set start date and end date if provided in URL
        if (params['startDate']) {
          $('#rp_from').val(params['startDate']);
          $('#rp_enabledate').prop('checked', true);
        }
        if (params['endDate']) {
          $('#rp_to').val(params['endDate']);
          $('#rp_enabledate').prop('checked', true);
        }
        
        // Set report selection if provided
        if (params['report']) {
          $('.menutab').removeClass('selected');
          $('#' + params['report']).addClass('selected');
          $('.tabplaceholder').css('display', 'none');
          $('#' + params['report'] + '-placeholder').css('display', '');
        }
    
        // Parse additional options
        if (params['openAps']) {
          $('#rp_optionsopenaps').prop('checked', params['openAps'] === 'true');
        }
      }
      // Auto-show the report if 'autoShow' parameter is present
      if (params['autoShow'] === 'true') {
        show();
      }

      // Hide menu elements if 'hideMenu' parameter is present
      if (hideMenu) {
      $('.menutab').hide(); // Hide all menus
     $('#rp_optionsform').hide(); // Hide any form sections
     // Hide other elements as needed
     $('#rp_show').hide();
     $('#info').hide();
     // Clear the page title if desired
     document.title = '';
      }
    ...
    function show (event) {
      var options = {
        ...
        cob: false,
        basal: true,
        scale: report_plugins.consts.SCALE_LINEAR, // or SCALE_LOG as desired
        weekscale: report_plugins.consts.SCALE_LINEAR,
        units: client.settings.units,
        openAps: true,
        insulindistribution: true,
        bgcheck: true,
        othertreatments: true,
        order: report_plugins.consts.ORDER_NEWESTONTOP,
      };
  ...
  if (hideMenu) {
    // Set date range from URL parameters
    var fromDateStr = params['startDate'];
    var toDateStr = params['endDate'];
    var from, to;
    if (fromDateStr && toDateStr) {
      from = moment.tz(moment(fromDateStr).startOf('day'), zone).startOf('day');
      to = moment.tz(moment(toDateStr).endOf('day'), zone).endOf('day');
    } else {
    ...
    matchesneeded = 1;
    daysfilter(); // Proceed directly to daysfilter
    // Set default options when hideMenu is true
    options.insulin = true;
    options.carbs = true;
    options.basal = true;
    options.notes = true;
    options.food = true;
    ...
    options.iob = false;
    options.cob = false;
    options.predicted = false;
    options.openAps = true;
    options.insulindistribution = true;
    options.bgcheck = true;
    options.othertreatments = true;
    options.scale = report_plugins.consts.SCALE_LINEAR; // or SCALE_LOG, as desired
    options.width = 1200;
    options.height = 400;
    options.order = report_plugins.consts.ORDER_NEWESTONTOP; // or ORDER_OLDESTONTOP
  } else {
    ...
    function processData (data, day, options, callback) {
      ...
      // Daniel: Round data.dailyCarbs to zero decimals
      data.dailyCarbs = parseFloat(data.dailyCarbs.toFixed(0));
