...
var init = function init () {
  ...
  if (params['openAps']) {
    $('#rp_optionsopenaps').prop('checked', params['openAps'] === 'true');
      ...
      // Auto-show the report and hide menu if 'hideMenu' parameter is present
if (params['hideMenu'] === 'true') {
  ...
  if (fromDateStr && toDateStr) {
    from = moment.tz(fromDateStr, zone).startOf('day');  // Set to start of the day
    to = moment.tz(toDateStr, zone).endOf('day');        // Set to end of the day
    
    // Make sure "to" does not exceed "from" (for single-day selection)
    if (fromDateStr === toDateStr) {
