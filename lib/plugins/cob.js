...
function init (ctx) {
  ...
  cob.cobTotal = function cobTotal (treatments, devicestatus, profile, time, spec_profile) {
    ...
    const FIFTEEN_MINUTES = 15 * 60 * 1000; //Test to fill gaps in Trio COB reporting

    if (_.isEmpty(result) || _.isNil(result.cob) || (time - result.mills) > FIFTEEN_MINUTES) { //Test to fill gaps in Trio COB reporting
