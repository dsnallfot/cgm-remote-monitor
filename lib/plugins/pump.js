...
var ALL_STATUS_FIELDS = ['reservoir', 'clock', 'status', 'device']; //, 'battery', 'clock', 'status', 'device'];
...
function init (ctx) {
  ...
  function updateReservoir(prefs, result) {
    if (result.reservoir) {
        ...
        if (result.reservoir.value === 0) {
            result.reservoir.display = '50+ E';
        } else if (result.reservoir.value < prefs.urgentRes) {
