...
var ALL_STATUS_FIELDS = ['reservoir', 'clock', 'status', 'device']; //, 'battery', 'clock', 'status', 'device']; // Daniel: Disabled all battery info
...
function init (ctx) {
  ...
  pump.getPrefs = function getPrefs (sbx) {
    ...
    retroFields = isEmpty(retroFields) ? ['reservoir'] : retroFields; //, 'battery'] : retroFields;
  ...
  function updateReservoir(prefs, result) {
    if (result.reservoir) {
        result.reservoir.label = 'Reservoar';
        result.reservoir.display = result.reservoir.value.toPrecision(3) + 'E';
    ...
    } else { //if (result.manufacturer === 'Insulet') { // fix for iAPS not uploading manufacter info
        result.reservoir = {
            label: 'Reservoar',
            display: '50+ E'
        }
  ...
  /*function updateBattery (type, prefs, result, batteryWarn) {
    if (result.battery) {
      result.battery.label = 'Batteristatus';
  ...
  function updateStatus(pump, result) {
    if (pump.status) {
      var status = pump.status.status || 'Status ok';
      if (pump.status.status === 'normal') { // Add OK when pump status is normal
        status = 'Normal';
      } else if (pump.status.bolusing) {
        status = 'Gav bolus';
      } else if (pump.status.suspended) {
        status = 'Avstängd!';
        if (pump.warnOnSuspend && pump.status.suspended) {
          result.status.level = levels.WARN;
          result.status.message = 'Pump avstängd';
        }
      }
  ...
  function prepareData (prop, prefs, sbx) {
    ...
    /*if (pump.battery && pump.battery.percent) {
      result.battery = { value: pump.battery.percent, unit: 'Procent' };
      updateBattery('%', prefs, result, batteryWarn);
    } else if (pump.battery && pump.battery.voltage) {
      result.battery = { value: pump.battery.voltage, unit: 'Volt'};
    ...
    if (openaps.findOfflineMarker(sbx)) {
      console.info('iAPS offline, larm pausade');
    } else {
      _.forEach(ALL_STATUS_FIELDS, function eachField(fieldName) {
        var field = result[fieldName];
  ...
  function timeFormat (m, sbx) {
    ...
    if (m && sbx.data.inRetroMode) {
      //when = m.format('LT');
      when = m.format('HH:mm'); //Daniel: Override with 24h format
    } else if (m) {
