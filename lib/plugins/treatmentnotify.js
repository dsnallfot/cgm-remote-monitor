...
const MANUAL_TREATMENTS = ['BG Check', 'Meal Bolus', 'Carb Correction', 'Kolhydrater', 'Måltid', 'Dextro', 'Profilbyte', 'Correction Bolus', 'Site Change', 'Sensor Start', 'Sensor Change', ];
...
function init(ctx) {
  ...
  function requestTreatmentNotify (lastTreatment, sbx) {
    ...
    if (lastTreatment.isAnnouncement) {
      ...
      if (!eventType && ['Sensor Change', 'Sensor Start', 'Site Change'].includes(lastTreatment.eventType)) {
          eventType = lastTreatment.eventType;
          }
...
function isCurrent(last) {
  ...
  return ago !== -1 && ago < times.mins(2).msecs;
}
