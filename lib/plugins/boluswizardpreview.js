...
function init (ctx) {
  ...
  bwp.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if (prop && prop.bolusEstimateDisplay >= 0) {
      value = prop.bolusEstimateDisplay + 'E';
  ...
  bwp.calc = function calc (sbx) {
    ...
    results.displayLine = translate('BWP') + ': ' + results.bolusEstimateDisplay + 'E';
  ...
  function pushInfo(prop, info, sbx) {
    if (prop && prop.errors) {
      ...
      info.push({label: translate('Insulin on Board'), value: prop.displayIOB + 'E'});
      ...
      info.push({label: translate('Current target'), value: sbx.data.profile.getLowBGTarget(sbx.time) + ' mmol'});
      info.push({label: translate('Sensitivity'), value: '-' + sbx.data.profile.getSensitivity(sbx.time) + ' ' + sbx.settings.units + '/E'});
      ...
      if (prop.bolusEstimateDisplay < 0) {
        ...
        info.unshift({label: translate('Carb Equivalent'), value: prop.bolusEstimateDisplay + 'E * ' + sbx.data.profile.getCarbRatio() + ' = ' + carbEquivalent + 'g'});
        info.unshift({label: translate('Current Carb Ratio'), value: '1E / ' + sbx.data.profile.getCarbRatio() + ' g'});
