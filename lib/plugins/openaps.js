...
function virtAsstForecastHandler(next, slots, sbx) {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    TO CONTINUE EXPERIMENTING WITH (Code below makes iAPS pill always visible, and prediction lines always present, but device status ValueParts only works for the latest entry)
if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(3, 'minutes'))) {
  ...
  // Handle historical events within the last 3 minutes here
  // You may retrieve historical data and push it to the 'events' array
}
