...
function init (ctx) {
  ...
  /*openaps.analyzeData = function analyzeData (sbx) {
    ...
    var recentData = _.chain(sbx.data.devicestatus)
      ...
      .value();*/
      //Test to add 3 min to analyze data
      openaps.analyzeData = function analyzeData(sbx) {
        var recentHours = 6;
        var recentMills = sbx.time - times.hours(recentHours).msecs - times.minutes(3).msecs;
...
/*events.push({
  time: prop.lastEnacted.moment.add(3, 'minutes'), // add 3 minutes
  value: valueParts.join('')
});*/
} else {
    addSuggestion();
