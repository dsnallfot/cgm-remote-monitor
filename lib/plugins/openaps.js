...
function init (ctx) {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    // Test to check if the event is 'enacted' and within the last 3 minutes
    ...
    /*if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(3, 'minutes'))) {
      ...
      // Handle delayed events here, maybe mark them as delayed or provide some indication
      if ('enacted' === prop.status.code) {
      // Consider marking the event as delayed or providing some indication
      } else {
      addSuggestion();
      }
    }*/
