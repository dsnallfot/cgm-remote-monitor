...
// Ensure "------------------------------ <br> ☑️ Oref slutsats:" appears at the right place
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/AF:\s([0-9]\.\d{1,2})(<br> • SMB Ratio:\s([0-9]\.\d{1,2}))?/g, function(match, afValue, smbRatio) {
  let result = `AF: ${afValue} <br>`; // AF always appears first
  if (smbRatio) {
      result += `• SMB Ratio: ${smbRatio} <br>`; // If SMB Ratio exists, place it directly after AF
  }
