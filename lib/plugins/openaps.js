...
/*
    ...
    if (moments.notEnacted && (
        (moments.enacted && (moments.notEnacted.isSameOrAfter(moments.enacted))) || 
        (!moments.enacted && moments.notEnacted.isSameOrAfter(recent))
    )) {
        //status.symbol = ' x ';
        status.symbol = ' ⚫️ ';
        status.code = 'notenacted';
...
_.forEach(recentData, function eachStatus (status) {
  ...
  openaps.checkNotifications = function checkNotifications (sbx) {
    ...
    if (!prop.lastLoopMoment) {
      console.info('Trio hasn\'t reported a loop yet');
      return;
    }
  ...
  openaps.findOfflineMarker = function findOfflineMarker (sbx) {
    return _.findLast(sbx.data.treatments, function match (treatment) {
      ...
      return eventTime <= sbx.time && treatment.eventType === 'Trio Offline' && eventEnd >= sbx.time;
    });
  };
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    function addSuggestion() {
      if (prop.lastSuggested) {
        ...
        var reasonWithLineBreaks = prop.lastSuggested.reason;
        ...
        reasonWithLineBreaks = reasonWithLineBreaks.replace(/AF:\s([0-9]\.\d{1,2})(?:,\sSMB Ratio:\s([0-9]\.\d{1,2}))?;/g, function(match, afValue, smbRatio) {
            ...
            return result + `------------------------------ <br><b>OREF SLUTSATS:</b><br> • `; // Ensure this is added once at the correct position
        });
        ...
        var valueParts = [
          //valueString('Trio ↻ Förslag (', localTimeString),
  ...
  var enactedReasonWithLineBreaks = prop.lastSuggested.reason;

// ✅ Step 1: Handle AF and SMB Ratio *before* replacing anything else
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/AF:\s([0-9]\.\d{1,2})(?:,\sSMB Ratio:\s([0-9]\.\d{1,2}))?;/g, function(match, afValue, smbRatio) {
  let result = `AF: ${afValue} <br>`; // AF always appears first
  if (smbRatio) {
      result += `• SMB Ratio: ${smbRatio} <br>`; // If SMB Ratio exists, place it directly after AF
  }
...
// ✅ Step 3: Continue with other replacements (won't affect AF/SMB placement)
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/SMB INAKTIVERADE!/g, 'SMB Inaktiverade 🚫');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/Mikrobolus:/g, '🔹 Mikrobolus:');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/\. ;/g, '<br>• ');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/E\. /g, 'E<br>');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/U\. /g, 'E<br>');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/E\/h\. /g, 'E/h<br>');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/temp\./g, 'temp.<br>');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/\. /g, '');

// ✅ Step 4: Replace "; " with <br> • (AF & SMB Ratio are already handled, so they won't be affected)
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/;\s+(?!\w*;|&\w+;)/g, ' <br> • ');

// ✅ Step 5: Other formatting rules
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/add'l carbs req w\/in/g, 'g kh behövs inom');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/TDD:\s(\d+(?:\.\d{1,2})?)\sU/g, '<b>TDD: $1E</b>');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/temp\s([0-9]+\.\d{1,2})&lt;([0-9]+\.\d{1,2})U\/hr\./g, 'Temp $1&lt;$2E/h');
enactedReasonWithLineBreaks = enactedReasonWithLineBreaks.replace(/insulinReq/g, 'Insulinbehov:');

 /*   
 var valueParts = [
  '<br>🔹 <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
  canceled ? '' : ' ' + (prop.lastEnacted.rate ?? 0) + 'E/h i ' + prop.lastEnacted.duration + 'm',
  valueString('<br><b>• BG: </b>', `<b>${bg}</b>`),
...
var valueParts = [
  ...
  events.push({
    ...
    if (forecastPoints && forecastPoints.length > 0) {
      sbx.pluginBase.addForecastPoints(forecastPoints, { type: 'Trio', label: 'Trio Forecasts' });
    }
  };
  ...
  openaps.virtAsst = {
    intentHandlers: [{
      intent: 'MetricNow'
      , metrics: ['Trio forecast', 'forecast']
  ...
  function statusLevel (prop, prefs, sbx) {
    ...
    if (openaps.findOfflineMarker(sbx)) {
      console.info('Trio known offline, not checking for alerts');
