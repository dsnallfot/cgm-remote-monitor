...
function init (ctx) {
  ...
  openaps.getEventTypes = function getEventTypes (sbx) {
    ...
    if (units == 'mmol') {
      ...
      reasonconf.push({ name: translate('Efter dextro'), targetTop: 10, targetBottom: 10, duration: 75 });
    } else {
      ...
      reasonconf.push({ name: translate('Efter dextro'), targetTop: 180, targetBottom: 180, duration: 75 });
    }
    return [
      {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    function addSuggestion () {
      if (prop.lastSuggested) {
        var bg = Math.round(prop.lastSuggested.bg / 18 * 10) / 10;
        var units = sbx.data.profile.getUnits();

        if (units === 'mmol') {
          bg = Math.round(prop.lastSuggested.bg / 18 * 10) / 10;
        }
