...
// Nedan kodblock är högintressant för att försöka lösa 3 min offset-problemet i iaps pill och latest enacted bg
_.forEach(recentData, function eachStatus(status) {
  ...
  // Update device status with enacted status if not more than 5 minutes old
  if (status.openaps && status.openaps.enacted) {
      var enacted = status.openaps.enacted;
      if ((!device.status || moments.when.isAfter(device.status.when)) &&
          (!result.lastEnacted || moments.enacted.isBefore(result.lastEnacted.moment)) &&
          moments.enacted.diff(moment(), 'minutes') <= 5) {
          device.status = loopStatus;
          device.status.when = moments.when;

          result.lastEnacted = moments.enacted;
  ...
  // Update lastNotEnacted status
  if (status.openaps && status.openaps.enacted && moments.notEnacted && (!result.lastNotEnacted || moments.notEnacted.isAfter(result.lastNotEnacted.moment))) {
      result.lastNotEnacted = moments.notEnacted;
  }

  // Update lastSuggested status
  if (status.openaps && status.openaps.suggested && moments.suggested && (!result.lastSuggested || moments.suggested.isAfter(result.lastSuggested.moment))) {
      var suggested = status.openaps.suggested;
      result.lastSuggested = moments.suggested;
...
if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(3, 'minutes'))) { //Tillägget på 3 min här verkar vara det som gör att predictions och device statusar alltid uppdateras samt finns tillgängliga historiskt. Dock så är det suggested som visas, Men 3 min buffert på enacted verkar göra att datat läses in och görs tillgänglig i historiken närm na scrollar i grafen
  ...
  /*if (prop.lastSuggested && prop.lastSuggested.moment.isAfter(prop.lastEnacted.moment.subtract(3, 'minutes'))) {
      addSuggestion();
  } else {
      valueParts = concatIOB(valueParts);
  }*/ //Detta test får enacted och allt BS att förskjutas 3 min
  ...
  //Test to always add bot addsuggestion and valueparts
  if (prop.lastSuggested && prop.lastSuggested.moment.isAfter(prop.lastEnacted.moment)) {
    addSuggestion();
  }
  ...
  events.push({
    time: prop.lastEnacted.moment, //.isAfter(moment().subtract(3, 'minutes')),
    value: valueParts.join('')
});
