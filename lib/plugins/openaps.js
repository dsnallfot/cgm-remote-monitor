...
function init (ctx) {
  ...
  openaps.analyzeData = function analyzeData (sbx) {
    ...
    function toMoments(status) {
      ...
      // Detect delayed enactments
      if (notEnacted && enacted && notEnacted.isAfter(enacted)) {
          delayedEnacted = notEnacted;
          notEnacted = false; // Reset notEnacted if it's a delayed enactment
      }
      ...
      return {
          ...
          enacted,
          notEnacted,
          delayedEnacted, // Store delayed enactments separately
          suggested,
          iob
      };
  }
    ...
    _.forEach(recentData, function eachStatus (status) {
      ...
      // Handle delayed enactments
      if (moments.delayedEnacted && (!result.lastDelayedEnacted || moments.delayedEnacted.isAfter(result.lastDelayedEnacted.moment))) {
        result.lastDelayedEnacted = moments.delayedEnacted;
      }
  ...
  openaps.updateVisualisation = function updateVisualisation(sbx) {
    ...
    function addEventToEvents(event) {
        events.push(event);
    }

  // Function to handle delayed events
function handleDelayedEvent() {
  if (prop.lastDelayedEnacted) {
      var bg = Math.round(prop.lastDelayedEnacted.bg / consts.MMOL_TO_MGDL * 10) / 10;

      var valueParts = [
          valueString('BG: ', bg),
          ', <b>Delayed Enactment</b>',
          valueString(', ', prop.lastDelayedEnacted.reason),
          prop.lastDelayedEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' <b>Meal Assist:</b> ' + prop.lastDelayedEnacted.mealAssist : ''
      ];

      addEventToEvents({
          time: prop.lastDelayedEnacted.moment,
          value: valueParts.join('')
      });
  }

  // Include all delayed events
  _.forEach(prop.delayedEvents, function(event) {
      var bg = Math.round(event.bg / consts.MMOL_TO_MGDL * 10) / 10;

      var valueParts = [
          valueString('BG: ', bg),
          ', <b>Delayed Enactment</b>',
          valueString(', ', event.reason),
          event.mealAssist ? ' <b>Meal Assist:</b> ' + event.mealAssist : ''
      ];
    ...
    // Function to handle 'enacted' events
    function handleEnactedEvent(prop) {
      var canceled = prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0;
      var bg = Math.round(prop.lastEnacted.bg / consts.MMOL_TO_MGDL * 10) / 10;

      var valueParts = [
          valueString('BG: ', bg),
          ', <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
      ...
      addEventToEvents({
          time: prop.lastEnacted.moment.subtract(5, 'minutes'), // Subtract 5 minutes for visualization
          value: valueParts.join('')
      });
  }
    ...
    function addSuggestion() {
      if (prop.lastSuggested) {
          var bg = Math.round(prop.lastSuggested.bg * 10) / 10;
          ...
          addEventToEvents({
              time: prop.lastSuggested.moment.subtract(5, 'minutes'), // Subtract 5 minutes for visualization
              value: valueParts.join('')
          });
      }
  }
    ...
    // Function to get forecast points
    ...
    // Handle 'enacted' events
    if ('enacted' === prop.status.code) {
        handleEnactedEvent(prop);
    } else {
        addSuggestion();
    }

    // Loop through seen devices
    ...
    // Map events to info objects
    var info = _.map(sortedEvents, function(event) {
        return {
    ...
    // Get forecast points and add them
...
// Function to handle virtual assistant forecast
function virtAsstForecastHandler(next, slots, sbx) {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if ('enacted' === prop.status.code) {
      ...
      var valueParts = [
        valueString('BG: ', Math.round(prop.lastEnacted.bg / consts.MMOL_TO_MGDL * 10) / 10)
        , ', <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>'
        , canceled ? '' : ' ' + prop.lastEnacted.rate.toFixed(2) + 'E/h i ' + prop.lastEnacted.duration + 'm'
        , valueString(', ', prop.lastEnacted.reason)
        , prop.lastEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' <b>Meal Assist:</b> ' + prop.lastEnacted.mealAssist : ''
