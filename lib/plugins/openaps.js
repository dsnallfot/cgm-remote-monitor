  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    function addSuggestion() {
      if (prop.lastSuggested) {
        ...
        // Parse the timestamp string and convert it to local time
        var localTime = new Date(timestamp);
        var localTimeString = localTime.toISOString().replace('T', ' ').replace(/\.\d+Z/, '');
        

        var valueParts = [
          valueString('iAPS ↻ Förslag<br> • BG: ', bg),
          valueString('<br> • ', reasonWithLineBreaks),
          valueString('<br> • <b>Info uppdaterad:</b> ', localTimeString)
