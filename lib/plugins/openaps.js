...
function init (ctx) {
  ...
  openaps.getEventTypes = function getEventTypes (sbx) {
    ...
    if (units == 'mmol') {
      ...
      reasonconf.push({ name: translate('Activity'), targetTop: 8, targetBottom: 8, duration: 120 });
      reasonconf.push({ name: translate('Efter dextro'), targetTop: 10, targetBottom: 10, duration: 45 });
    } else {
      reasonconf.push({ name: translate('Eating Soon'), targetTop: 72, targetBottom: 72, duration: 60 });
      reasonconf.push({ name: translate('Activity'), targetTop: 144, targetBottom: 144, duration: 120 });
      reasonconf.push({ name: translate('Efter dextro'), targetTop: 180, targetBottom: 180, duration: 45 });
    }
    return [
      {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    function addSuggestion () {
      if (prop.lastSuggested) {
        var bg = Math.round(prop.lastSuggested.bg / constants.MMOL_TO_MGDL * 10) / 10;
        var units = sbx.data.profile.getUnits();

        if (units === 'mmol') {
          bg = Math.round(prop.lastSuggested.bg / constants.MMOL_TO_MGDL * 10) / 10;
        }
