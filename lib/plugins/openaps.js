  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    function addSuggestion() {
      if (prop.lastSuggested) {
        ...
        // Replace all commas with line breaks        var reasonWithLineBreaks = prop.lastSuggested.reason.replace(/,/g, ' <br> • ');
        ...
        var valueParts = [
          valueString('iAPS ↻ Förslag<br> • BG: ', bg),
    ...
    function concatIOB (valueParts) {
      if (prop.lastIOB) {
        valueParts = valueParts.concat([
          ...
          , prop.lastIOB.basaliob ? '<br>Basal IOB ' + sbx.roundInsulinForDisplayFormat(prop.lastIOB.basaliob) + 'E' : ''
