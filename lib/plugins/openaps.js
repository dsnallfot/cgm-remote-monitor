...
if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(3, 'minutes'))) {
  ...
  if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(3, 'minutes'))) {
      // Include prop.lastEnacted.reason in historical data
      var valueParts = [
          valueString('BG: ', prop.lastEnacted.bg),
          ', <b>Temp Basal' + (prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0 ? ' Avbruten' : ' Startad') + '</b>',
          prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0 ? '' : ' ' + prop.lastEnacted.rate.toFixed(2) + 'E/h i ' + prop.lastEnacted.duration + 'm',
          valueString(', ', prop.lastEnacted.reason),
          prop.lastEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' <b>Meal Assist:</b> ' + prop.lastEnacted.mealAssist : ''
      ];
      ...
      // Handle other historical events here if needed
  }
}
