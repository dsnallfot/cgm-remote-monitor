...
_.forEach(recentData, function eachStatus (status) {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    function addSuggestion () {
      ...
      /*if (prop.lastSuggested && prop.lastSuggested.moment.isAfter(moment().subtract(3, 'minutes'))) {
        var bg = prop.lastSuggested.bg;
        var units = sbx.data.profile.getUnits();*/ // Detta gör att inga historiska device status-texter visas. bara att enacted skett
        ...
        events.push({
          time: prop.lastSuggested.moment, //.add(3, 'minutes'), // add 3 minutes
    ...
    function concatIOB (valueParts) {
      if (prop.lastIOB) {
        valueParts = valueParts.concat([
          ' • <b>IOB:</b> '
          , sbx.roundInsulinForDisplayFormat(prop.lastIOB.iob) + 'E'
          , prop.lastIOB.basaliob ? ', Basal IOB ' + sbx.roundInsulinForDisplayFormat(prop.lastIOB.basaliob) + 'E' : ''
          , prop.lastIOB.bolusiob ? ', Bolus IOB ' + sbx.roundInsulinForDisplayFormat(prop.lastIOB.bolusiob) + 'E.' : ''
        ]);
      }
...
// Daniel: Check if the event is 'enacted' on time (original code)
if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment())) { 
  ...
  var valueParts = [
      valueString('• <b>BG:</b> ', bg),
      ' • (Test 0) <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
...
// Daniel Test check 1: Check if the event is 'enacted' and within the last 1 minutes
} else if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(1, 'minutes'))) { //Tillägget på x min här verkar vara det som gör att predictions och device statusar alltid uppdateras samt finns tillgängliga historiskt. Dock så är det suggested som visas, Men 3 min buffert på enacted verkar göra att datat läses in och görs tillgänglig i historiken närm na scrollar i grafen
  var canceled = prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0;
  var bg = prop.lastEnacted.bg;
  var units = sbx.data.profile.getUnits(); 
  ...
  var valueParts = [
      valueString('• <b>BG:</b> ', bg),
      ' • (Test 1) <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
      canceled ? '' : ' ' + prop.lastEnacted.rate.toFixed(2) + 'E/h i ' + prop.lastEnacted.duration + 'm',
      valueString(', ', prop.lastEnacted.reason),
      prop.lastEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' • <b>Meal Assist:</b> ' + prop.lastEnacted.mealAssist : ''
  ];
...
// Daniel Test check 2: Check if the event is 'enacted' and within the last 2 minutes
} else if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(2, 'minutes'))) { //Tillägget på x min här verkar vara det som gör att predictions och device statusar alltid uppdateras samt finns tillgängliga historiskt. Dock så är det suggested som visas, Men 3 min buffert på enacted verkar göra att datat läses in och görs tillgänglig i historiken närm na scrollar i grafen
  var canceled = prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0;
  var bg = prop.lastEnacted.bg;
  var units = sbx.data.profile.getUnits(); 
  ...
  var valueParts = [
      valueString('• <b>BG:</b> ', bg),
      ' • (Test 2) <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
      canceled ? '' : ' ' + prop.lastEnacted.rate.toFixed(2) + 'E/h i ' + prop.lastEnacted.duration + 'm',
      valueString(', ', prop.lastEnacted.reason),
      prop.lastEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' • <b>Meal Assist:</b> ' + prop.lastEnacted.mealAssist : ''
  ];
...
// Daniel Test check 3: Check if the event is 'enacted' and within the last 3 minutes
} else if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(3, 'minutes'))) { //Tillägget på x min här verkar vara det som gör att predictions och device statusar alltid uppdateras samt finns tillgängliga historiskt. Dock så är det suggested som visas, Men 3 min buffert på enacted verkar göra att datat läses in och görs tillgänglig i historiken närm na scrollar i grafen
  var canceled = prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0;
  var bg = prop.lastEnacted.bg;
  var units = sbx.data.profile.getUnits(); 
  ...
  var valueParts = [
      valueString('• <b>BG:</b> ', bg),
      ' • (Test 3) <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
      canceled ? '' : ' ' + prop.lastEnacted.rate.toFixed(2) + 'E/h i ' + prop.lastEnacted.duration + 'm',
      valueString(', ', prop.lastEnacted.reason),
      prop.lastEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' • <b>Meal Assist:</b> ' + prop.lastEnacted.mealAssist : ''
  ];
...
// Daniel Test check 4: Check if the event is 'enacted' and within the last 4 minutes
} else if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(4, 'minutes'))) { //Tillägget på x min här verkar vara det som gör att predictions och device statusar alltid uppdateras samt finns tillgängliga historiskt. Dock så är det suggested som visas, Men 3 min buffert på enacted verkar göra att datat läses in och görs tillgänglig i historiken närm na scrollar i grafen
  var canceled = prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0;
  var bg = prop.lastEnacted.bg;
  var units = sbx.data.profile.getUnits(); 
  ...
  var valueParts = [
      valueString('• <b>BG:</b> ', bg),
      ' • (Test 4) <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
      canceled ? '' : ' ' + prop.lastEnacted.rate.toFixed(2) + 'E/h i ' + prop.lastEnacted.duration + 'm',
      valueString(', ', prop.lastEnacted.reason),
      prop.lastEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' • <b>Meal Assist:</b> ' + prop.lastEnacted.mealAssist : ''
  ];
...
// Daniel Test check 5: Check if the event is 'enacted' and within the last 5 minutes
} else if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(5, 'minutes'))) { //Tillägget på x min här verkar vara det som gör att predictions och device statusar alltid uppdateras samt finns tillgängliga historiskt. Dock så är det suggested som visas, Men 3 min buffert på enacted verkar göra att datat läses in och görs tillgänglig i historiken närm na scrollar i grafen
  var canceled = prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0;
  var bg = prop.lastEnacted.bg;
  var units = sbx.data.profile.getUnits(); 
  ...
  var valueParts = [
      valueString('• <b>BG:</b> ', bg),
      ' • (Test 5) <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
      canceled ? '' : ' ' + prop.lastEnacted.rate.toFixed(2) + 'E/h i ' + prop.lastEnacted.duration + 'm',
      valueString(', ', prop.lastEnacted.reason),
      prop.lastEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' • <b>Meal Assist:</b> ' + prop.lastEnacted.mealAssist : ''
  ];
...
// Daniel Test check 6: Check if the event is 'enacted' and within the last 6 minutes
} else if ('enacted' === prop.status.code && prop.lastEnacted.moment.isAfter(moment().subtract(6, 'minutes'))) { //Tillägget på x min här verkar vara det som gör att predictions och device statusar alltid uppdateras samt finns tillgängliga historiskt. Dock så är det suggested som visas, Men 3 min buffert på enacted verkar göra att datat läses in och görs tillgänglig i historiken närm na scrollar i grafen
  var canceled = prop.lastEnacted.rate === 0 && prop.lastEnacted.duration === 0;
  var bg = prop.lastEnacted.bg;
  var units = sbx.data.profile.getUnits(); 
  ...
  var valueParts = [
      valueString('• <b>BG:</b> ', bg),
      ' • (Test 6) <b>Temp Basal' + (canceled ? ' Avbruten' : ' Startad') + '</b>',
      canceled ? '' : ' ' + prop.lastEnacted.rate.toFixed(2) + 'E/h i ' + prop.lastEnacted.duration + 'm',
      valueString(', ', prop.lastEnacted.reason),
      prop.lastEnacted.mealAssist && _.includes(selectedFields, 'meal-assist') ? ' • <b>Meal Assist:</b> ' + prop.lastEnacted.mealAssist : ''
  ];
