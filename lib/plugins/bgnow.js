  ...
  bgnow.updateVisualisation = function updateVisualisation(sbx) {
    ...
    if (delta && delta.interpolated) {
        ...
        info.push({
            ...
            value: sbx.roundBGToDisplayFormat(sbx.scaleMgdl(delta.absolute)) + ' ' + sbx.unitsLabel
        });
        info.push({
            label: translate('Interpolated'),
            value: sbx.roundBGToDisplayFormat(sbx.scaleMgdl(delta.mean5MinsAgo)) + ' ' + sbx.unitsLabel
        });
    }
    ...
    if (delta && delta.previous && delta.previous.sgvs) {
        _.forEach(delta.previous.sgvs, function deviceAndValue(entry) {
            ...
            if (deviceInfo && deviceInfo.recent) {
                var deviceDelta = bgnow.calcDelta({
                        mills: deviceInfo.recent.mills,
                        mean: deviceInfo.recent.mgdl
                    },
                    {
                        mills: entry.mills,
                        mean: entry.mgdl
        ...
        if (_.keys(deviceInfos).length > 1) {
            _.forIn(deviceInfos, function addInfo(deviceInfo, name) {
                ...
                // Apply the regex replace here just before adding to the tooltip
                name = name.replace(/share2/g, "Dexcom");
                info.push({
                    label: name,
                    value: display
                });
