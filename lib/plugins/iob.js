...
function init(ctx) {
  ...
  iob.updateVisualisation = function updateVisualisation(sbx) {
    ...
    if (prop.lastBolus) {
      ...
      var amount = sbx.roundInsulinForDisplayFormat(Number(prop.lastBolus.insulin)) + 'E';
    ...
    if (prop.bolusiob !== undefined) {
      info.push({ label: translate('Bolus IOB'), value: prop.bolusiob.toFixed(2) });
    }
    if (prop.source !== undefined) {
      info.push({ label: translate('Source'), value: prop.source });
    }
    var value = (prop.display !== undefined ? sbx.roundInsulinForDisplayFormat(prop.display) : '---') + 'E';
