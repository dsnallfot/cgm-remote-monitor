...
const treatmentNotify = require('../lib/plugins/treatmentnotify'); // Adjust the path as needed
...
function init (env, ctx) {
  ...
  var recentlySent = new NodeCache({ stdTTL: times.mins(1).secs, checkperiod: 20 });
  ...
  function sendMakerEvent(notify) {
    ...
    var lasttreatTimestamp = treatmentNotify.timestamp;
    ...
    ctx.maker.sendEvent(event, function makerCallback (err) {
      if (err) {
        ...
        recentlySent.ttl(notify.key, times.mins(1).secs);
      }
    });
  }
