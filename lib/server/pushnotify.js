...
function init (env, ctx) {
  ...
  var recentlySent = new NodeCache({ stdTTL: times.mins(5).secs, checkperiod: 20 });
  ...
  function sendMakerEvent(notify) {
    ...
    var key = notify.key; // Retrieve the key from the notification object
    ...
    ctx.maker.sendEvent(event, function makerCallback(err) {
      if (err) {
        ...
        recentlySent.set(key, notify, times.mins(9).secs); // Set the TTL for the specific key to 5 minutes
      }
    });
  }
