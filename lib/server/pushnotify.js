...
function init (env, ctx) {
  ...
  var recentlySent = new NodeCache({ stdTTL: times.mins(10).secs, checkperiod: 20 });
  ...
  function sendMakerEvent(notify) {
    ...
    ctx.maker.sendEvent(event, function makerCallback (err) {
      if (err) {
        ...
        recentlySent.ttl(notify.key, times.mins(10).secs);
      }
    });
  }
